...
class Intra3D(Dataset):
    '''
    process Intra data and generate dataloader
    '''
    def __init__(self, train_mode='train', cls_state=True, npoints=2048, data_aug=True, choice=0):
        ...
        # get labels from data path, split train set and test according to files in /Data/IntrA3D/fileSplit
        if self.cls_state:
        ...
        # get the storage path of train and test data
    ...
    def __getitem__(self, index):
        ...
        # get label
        if self.cls_state:
            if curr_file.split('/')[-3] == 'vessel':
                cls = torch.from_numpy(np.array([0]).astype(np.int64))
            elif curr_file.split('/')[-3] in ['aneurysm', 'annotated']:
