...
class MGR(nn.Module):
    '''
    Multi-Graph Reasoning net (Using attention mechanism)
    '''
...
class Model(nn.Module):
    ...
    build 3DMedPT
    '''
    def __init__(self, args, trans_block, output_channels=2, dataset='Intra3D'):
        super().__init__()
        ...
        # set dataset 1=Intra3D, 0=Aneurisk
        self.dataset = (dataset == 'Intra3D') + 0
    ...
    def forward(self, x):
        ...
        xyz = x[..., :3]
        if not self.use_norm or self.dataset == 0:
            feature = xyz
        else:
            assert x.size()[-1] == 6
            feature = x[..., 3:]
