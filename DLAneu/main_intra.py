...
def _init_():
    '''
    Check and create the folders needed to run
    '''
...
def train_dbn(args, io, split, dataset='Intra3D'):
    '''
    train DBN
    '''
    # get train data
    ...
    elif dataset == 'aneurisk':
        train_loader = DataLoader(
            Aneurisk(train_mode='train', cls_state=True, npoints=args.num_points, data_aug=True),
            num_workers=0, batch_size=1, shuffle=True, drop_last=True)
    else:
        print('ERROR')
    ...
    rbm1 = BernoulliRBM(n_components=100, learning_rate=0.09, n_iter=150, verbose=1, random_state=101)
    rbm2 = BernoulliRBM(n_components=80, learning_rate=0.09, n_iter=150, verbose=1, random_state=101)
    rbm3 = BernoulliRBM(n_components=80, learning_rate=0.09, n_iter=150, verbose=1, random_state=101)
    model = Pipeline(steps=[('rbm1', rbm1), ('rbm2', rbm2), ('rbm3', rbm3), ('logistic', logistic)])
    labels = []
    first_flag = True
    ...
    for batch_data in tqdm(train_loader, total=len(train_loader)):
        ...
        xyz = data[..., :3].to(device)
        feature = data[..., 3:].to(device)
        fps_idx = pt.furthest_point_sample(xyz.contiguous(), 512).to(device).long()  # [B, S]
        new_xyz = index_points(xyz, fps_idx).to(device)  # [B, S, 3]
        if dataset == 'Intra3D':
            new_feature = index_points(feature, fps_idx).to(device).transpose(2, 1).contiguous()  # [B, C, S]

        knn_idx = knn_point(32, xyz, new_xyz).to(device)  # [B, S, K]
        neighbor_xyz = index_points(xyz, knn_idx).to(device)  # [B, S, K, 3]

        if dataset == 'Intra3D':
            grouped_features = index_points(feature, knn_idx).to(device)  # [B, S, K, C]
            grouped_features = grouped_features.to(device).permute(0, 3, 1, 2).contiguous()  # [B, C, S, K]
            grouped_points_norm = grouped_features - new_feature.unsqueeze(-1).contiguous()  # [B, C, S, K]
        # relative spatial coordinates
        relative_pos = neighbor_xyz - new_xyz.unsqueeze(-2).repeat(1, 1, 32, 1)  # [B, S, K, 3]
        relative_pos = relative_pos.permute(0, 3, 1, 2).contiguous()  # [B, 3, S, K]

        if dataset == 'Intra3D':
            feature = torch.cat([grouped_points_norm,
                                 new_feature.unsqueeze(-1).repeat(1, 1, 1, 32),
                                 relative_pos], dim=1)
        else:
            neighbor_xyz = neighbor_xyz.to(device).permute(0, 3, 1, 2).contiguous()
            feature = torch.cat([neighbor_xyz,
                                 relative_pos], dim=1)
        if first_flag:
            train_data = feature
            first_flag = False
        else:
            train_data = torch.cat((train_data, feature), 0)
    ...
    train_data = train_data.reshape(size, -1)
    # train DBN
    model.fit(train_data, labels)
    # save model
    if dataset == 'Intra3D':
        joblib.dump(model, '.\checkpoints\dbn_intra_train\models\dbm_0.pkl')
    else:
        joblib.dump(model, '.\checkpoints\dbn_aneurisk_train\models\dbm_0.pkl')
    # Calculate accuracy
    acc = model.score(train_data, labels)
    print(acc)


def test_dbn(args, io, split, dataset='Intra3D'):
    '''
    test DBN
    '''
    # get test data
    if dataset == 'Intra3D':
        ...
        test_loader = DataLoader(
            Aneurisk(train_mode='test', cls_state=True, npoints=args.num_points, data_aug=True),
            num_workers=0, batch_size=1, shuffle=True, drop_last=True)
    else:
        print('ERROR')
    device = torch.device("cuda" if args.cuda else "cpu")
    ...
    for batch_data in tqdm(test_loader, total=len(test_loader)):
        ...
        if first_flag:
            ...
            test_data = torch.cat((test_data, feature), 0)
        labels.append(label.item())
    labels = np.array(labels)
    size = test_data.shape[0]
    test_data = test_data.to('cpu').numpy()
    test_data = np.mean(test_data, axis=-1)
    test_data = test_data.reshape(size, -1)
    # load model
    if dataset == 'Intra3D':
        model = joblib.load('.\checkpoints\dbn_intra_train\models\dbm_0.pkl')
    else:
        model = joblib.load('.\checkpoints\dbn_aneurisk_train\models\dbm_0.pkl')
    # predict
    acc = model.score(test_data, labels)
    print(acc)


def train(args, io, split, num_class=2, dataset='Intra3D'):
    '''
    train 3DMedPT
    '''
    # get train data
    if dataset == 'Intra3D':
        ...
        train_loader = DataLoader(
            Aneurisk(train_mode='train', cls_state=True, npoints=args.num_points, data_aug=True),
            num_workers=0, batch_size=2, shuffle=True, drop_last=True)
        test_loader = DataLoader(
            Aneurisk(train_mode='test', cls_state=True, npoints=args.num_points, data_aug=True),
            num_workers=0, batch_size=2, shuffle=True, drop_last=True)
    ...
    args.device = device
    # init model
    model = Model(args, transformer, num_class, dataset).to(device)
    ...
    # train 3DMedPT and show the process
...
def val(test_loader, num_class, model, device, epoch, best_V_acc,
    ...
    '''
    Validating training results and decide whether to save the model
    '''
    test_pred = []
    test_true = []
...
def test(args, io, split, num_class=2, dataset='Intra3D'):
    '''
    test 3DMedPT
    '''
    # get dataset
    ...
    if args.use_gpus:
        model = nn.DataParallel(model)
    if dataset == 'Intra3D':
        path = './checkpoints/%strain/models/model_split_%d_f1.pth' % (args.exp_name[:-5], split)
    else:
        path = './checkpoints/aneurisk_train/models/model_split_0_f1.pth'
...
if __name__ == "__main__":
    ...
    # train or test
    if args.exp_name == 'intra_cls_train':
        for split in range(1):
            ...
            if args.mode == 'train':
                ...
                print("Error")
                exit(-1)
    elif args.exp_name == 'dbn_intra_train':
        if args.mode == 'train':
            train_dbn(args, io, 0)
        elif args.mode == 'test':
            test_dbn(args, io, 0)
            ...
            print('ERROR')
    elif args.exp_name == 'aneurisk_train':
        split = 0
        if args.mode == 'train':
            train(args, io, 0, 2, 'aneurisk')
        elif args.mode == 'test':
            test(args, io, 0, 2, 'aneurisk')
        else:
            print('ERROR')
    elif args.exp_name == 'dbn_aneurisk_train':
        if args.mode == 'train':
            train_dbn(args, io, 0, 'aneurisk')
        elif args.mode == 'test':
            test_dbn(args, io, 0, 'aneurisk')
        else:
            print('ERROR')
    else:
        print('ERROR')
